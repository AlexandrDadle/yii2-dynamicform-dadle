...
(function ($) {
    ...
    var _resolveWidgetOptions = function (attr) {
        if (!attr) {
            console.error('data-dynamicform attribute is missing.');
            return null;
        }

        var key = $.trim(attr);
        if (!key.length) {
            console.error('data-dynamicform attribute is empty.');
            return null;
        }
        ...
        if (key.charAt(0) === '{' || key.charAt(0) === '[') {
            try {
                ...
                console.error('Failed to parse widget options JSON:', err);
                return null;
            }
        }

        var options = window[key];
        if (!options) {
            console.error('Dynamic form options not found for key:', key);
            return null;
        }
    ...
    var methods = {
        ...
        updateContainer: function () {
            var widgetOptions = _resolveWidgetOptions($(this).attr('data-dynamicform'));
            if (!widgetOptions) {
                return;
            }
    ...
    var _getWidgetOptionsRoot = function (widgetOptions) {
        var rootOptions = _resolveWidgetOptions($(widgetOptions.widgetBody).parents('div[data-dynamicform]').last().attr('data-dynamicform'));
        return rootOptions;
    };
    ...
    var _removeValidations = function ($elem, widgetOptions, count) {
        if (count > 1) {
            $elem.find('div[data-dynamicform]').each(function () {
                var currentWidgetOptions = _resolveWidgetOptions($(this).attr('data-dynamicform'));
                if (!currentWidgetOptions) {
                    return;
                }
                ...
                for (var i = 1; i <= numItems - 1; i++) {
                    ...
                    currentWidgetOptions.fields.forEach(function (input) {
                        ...
                        if (typeof $("#" + currentWidgetOptions.formId).yiiActiveForm("find", id) !== 'undefined') {
            ...
            widgetOptions.fields.forEach(function (input) {
                ...
                if (typeof $("#" + widgetOptions.formId).yiiActiveForm("find", id) !== 'undefined') {
    ...
    var _updateAttrID = function ($elem, index) {
        var widgetOptions = _resolveWidgetOptions($elem.closest('div[data-dynamicform]').attr('data-dynamicform'));
        if (!widgetOptions) {
            return;
        }
        var id = $elem.attr('id');
            ...
            if (matches && matches.length === 4) {
                ...
                if (identifiers.length > 1) {
                    ...
                    $elem.parents('div[data-dynamicform]').each(function (i) {
                        var options = _resolveWidgetOptions($(this).attr('data-dynamicform'));
                        if (!options) {
                            parseFailed = true;
                            return false;
                        }
                        widgetsOptions[i] = options;
                    });
    ...
    var _fixFormValidaton = function (widgetOptions) {
        ...
        $(widgetOptionsRoot.widgetBody).find('input, textarea, select').each(function () {
            ...
            if (id !== undefined && name !== undefined) {
                var currentWidgetOptions = _resolveWidgetOptions($(this).closest('div[data-dynamicform]').attr('data-dynamicform'));
                if (!currentWidgetOptions) {
                    return;
                }
                var matches = id.match(regexID);
